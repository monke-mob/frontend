<script lang="ts">
    // This determines if the landing will display an announcement or branding.
    const isAnnouncement: boolean = false;
</script>

<div class="w-screen h-screen flex flex-col gap-4 justify-center items-center relative">
    {#if isAnnouncement}
        <img class="h-36" src="/images/logoText.svg" alt="monke mob" />

        <!--Change this text for announcements.-->
        <p class="text-2xl text-light">Closed Alpha Release<br />March 10th 2024</p>

        <div class="flex mt-6">
            <a class="button" href="/">Learn more</a>
        </div>
    {:else}
        <div class="w-[45vw] h-[65vh] absolute right-0 top-1/2 -translate-y-1/2">
            {#each Array(4) as _value, quadrant}
                <div class="absolute w-1/2 h-1/2" style="top: calc({25 * (quadrant == 0 || quadrant == 1 ? 0 : 2)}% - 20px); left: calc({25 * (quadrant == 0 || quadrant == 2 ? 0 : 2)}% - 20px);">
	...
	C104.4,94.4,109.6,95.8,115.7,103.1L115.7,103.1z"
        ...
        <div class="absolute left-40">
            <p class="text-2xl text-light">going bananas 🍌</p>
        </div>
    {/if}
</div>
